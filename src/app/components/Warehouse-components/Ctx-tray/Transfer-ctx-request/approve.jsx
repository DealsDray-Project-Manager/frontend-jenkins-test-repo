    ...
    const fetchData = async () => {
      try {
        let response = await axiosWarehouseIn.post(
          "/getWhtTrayItem/" + trayId + "/" + "Sales Transfer Request sent to Warehouse"
        );
  ...
  const handelIssue = async (e, sortId) => {
    try {
        ...
        let obj = {
          ...
          sortId: "Transferred to Sales",
          sales_location:trayData?.recommend_location,
           page:"Warehouse-approve"
        };
        let res = await axiosWarehouseIn.post("/ctx/transferRequest/approve", obj);
        if (res.status == 200) {
          ...
          Swal.fire({
            ...
            setLoading(false);
            navigate("/wareshouse/ctx/transfer/request");
      ...
      <div style={{ float: "right" }}>
        <Box sx={{ float: "right" }}>
          ...
          <Button
            sx={{ m: 3, mb: 9 }}
            variant="contained"
            disabled={loading == true || trayData?.actual_items?.length !== trayData?.items?.length  || description == "" ? true : false}
