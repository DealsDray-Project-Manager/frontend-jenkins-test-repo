...
// RDL_one_done_send_to_warehouse
// let res = await axiosWarehouseIn.post(
//     '/request-for-RDL_one/' + 'RDL_one_done_send_to_warehouse/' + location
// )
    ...
    const addActualitem = async (obj) => {
        if (trayData.items.length < trayData?.actual_items?.length) {
            ...
            try {
                ...
                setTextDisable(true)
                let res = await axiosWarehouseIn.post(
                    '/wht-add-actual-item-rdl-return',
                    objData
                )
                if (res.status == 200) {
                    setUic('')
    ...
    const handelIssue = async (e, sortId) => {
        try {
            if (trayData?.actual_items?.length == trayData?.items?.length) {
                ...
                let res = await axiosWarehouseIn.post(
                    '/issue-to-agent-wht-recived-rdl-one',
                    obj
                )
                if (res.status == 200) {
                    Swal.fire({
