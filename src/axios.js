import axios from 'axios'
export const axiosSuperAdminPrexo = axios.create({
    baseURL: 'http://prexo-v7-1-dev-api.dealsdray.com/api/v7/superAdmin',
    headers: { 'x-access-token': localStorage.getItem('prexo-authentication') },
})
export const axiosMisUser = axios.create({
    baseURL: 'http://prexo-v7-1-dev-api.dealsdray.com/api/v7/mis',
    headers: { 'x-access-token': localStorage.getItem('prexo-authentication') },
})
export const axiosWarehouseIn = axios.create({
    baseURL: 'http://prexo-v7-1-dev-api.dealsdray.com/api/v7/warehouseIn',
    headers: { 'x-access-token': localStorage.getItem('prexo-authentication') },
})
export const axiosBot = axios.create({
    baseURL: 'http://prexo-v7-1-dev-api.dealsdray.com/api/v7/bot',
    headers: { 'x-access-token': localStorage.getItem('prexo-authentication') },
})
export const axiosCharging = axios.create({
    baseURL: 'http://prexo-v7-1-dev-api.dealsdray.com/api/v7/charging',
    headers: { 'x-access-token': localStorage.getItem('prexo-authentication') },
})
export const axiosBqc = axios.create({
    baseURL: 'http://prexo-v7-1-dev-api.dealsdray.com/api/v7/bqc',
    headers: { 'x-access-token': localStorage.getItem('prexo-authentication') },
})
export const axiosSortingAgent = axios.create({
    baseURL: 'http://prexo-v7-1-dev-api.dealsdray.com/api/v7/sorting-agnet',
    headers: { 'x-access-token': localStorage.getItem('prexo-authentication') },
})
export const axiosAuditAgent = axios.create({
    baseURL: 'http://prexo-v7-1-dev-api.dealsdray.com/api/v7/audit-agent',
    headers: { 'x-access-token': localStorage.getItem('prexo-authentication') },
})
